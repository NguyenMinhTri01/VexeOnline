<template>
  <div class="result-ticket">
    <div v-if="loadingResult" class="notication">
      <Loader v-if="loading" />
      ...
      <div class="back-ground">
        <img
          src="https://res.cloudinary.com/vexeonline/c_scale,w_850/VexeOnlineMedia/imageDefault/bus_elxsln"
          alt
        />
      </div>
    </div>
    <div v-else class="content-ticket">
      ...
      <div class="customer-info">
        <div class="row">
          ...
          <div class="col-sm-4">
            <p>
              <strong>Điện Thoại:</strong> {{ticket.phone}}
            </p>
            <p>
...
export default {
  data() {
    return {
      loadingResult: true,
      notification: "Vui lòng nhập thông tin và bấm kiểm tra vé", //Mã vé hoặc số điện thoại không đúng. Vui lòng thử lại
      ticket : null,
    ...
    data(value) {
      if (value) {
        if (value.err) {
          ...
          this.notification = "Mã vé hoặc số điện thoại không đúng. Vui lòng thử lại";
        } else {
          moment.locale("vi");
          this.ticket = {...value}
